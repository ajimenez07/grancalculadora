...
#ifdef DEBUG
static std::string
token_type_to_string (Token::TokenType type)
{
  switch (type)
    {
...
struct ParserCursor
{
  ...
  explicit ParserCursor (std::vector<Token> tokens) : tokens (tokens) {}
};
...
parse_primary (ParserCursor &cursor)
{
  ...
  // +x or -x
  if (tk.type == Token::TokenType::PLUS || tk.type == Token::TokenType::MINUS)
    {
      ...
      parent->op = (tk.type == Token::TokenType::PLUS) ? UnaryExpr::Op::PLUS
                                                       : UnaryExpr::Op::MINUS;
      return parent;
    }
...
parse_term (ParserCursor &cursor)
{
  ...
  if (tk.type == Token::TokenType::MUL || tk.type == Token::TokenType::DIV)
    {
      ...
      bin->op = (tk.type == Token::TokenType::MUL) ? BinaryExpr::Op::MUL
                                                   : BinaryExpr::Op::DIV;
  ...
  if (tk.type == Token::TokenType::PLUS || tk.type == Token::TokenType::MINUS)
    {
      ...
      bin->op = (tk.type == Token::TokenType::PLUS) ? BinaryExpr::Op::ADD
                                                    : BinaryExpr::Op::SUB;
