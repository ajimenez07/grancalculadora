...
FractionElement::update_geometry (const Cairo::RefPtr<Cairo::Context> &cr)
{
  ...
  double den_total_width
      = denominator->get_width () + denominator->get_margin () * 2;
  double num_total_width
      = numerator->get_width () + numerator->get_margin () * 2;

  double width = (den_total_width > num_total_width) ? den_total_width
                                                     : num_total_width;

  // calculate total height of the denominator and numerator boxes
  double den_total_height
      = denominator->get_height () + denominator->get_margin () * 2;
  double num_total_height
      = numerator->get_height () + numerator->get_margin () * 2;
  ...
  denominator->geometry.x
      = (width - den_total_width) / 2 + denominator->get_margin ();
  numerator->geometry.x
      = (width - num_total_width) / 2 + numerator->get_margin ();
}
...
PowerElement::update_geometry (const Cairo::RefPtr<Cairo::Context> &cr)
{
  ...
  geometry.width = base->get_width () + exponent->get_width ()
                   + base->get_margin () * 2 + exponent->get_margin () * 2;
  geometry.height = base->get_height () + exponent->get_height ()
                    + base->get_margin () * 2 + exponent->get_margin () * 2;
  // set positions
  base->geometry.x = geometry.margin;
  base->geometry.y = geometry.margin;
  exponent->geometry.x
      = base->geometry.x + base->geometry.width + base->geometry.margin;
  exponent->geometry.y
      = base->geometry.y + base->geometry.height + base->geometry.margin;
}
  ...
  if (cursor.idx >= cursor.expr->elements.size () && !cursor.parents.empty ())
    {
      ...
      // do the same as we do in move_left but into the last element of the
      // numerator see 'Display::move_left' comments.
      if (el->type == ElementType::FRACTION)
        {
          if (!cursor.data.in_numerator)
            {
...
if we are really in a fraction. Undefined behavior may occur if this is not
done. */
void
...
void
draw_elements (const Cairo::RefPtr<Cairo::Context> &cr, Expr *expr, double dx,
               double dy, int width, int height, Cursor cursor)
{
  ...
  if (cursor.expr->elements.size ()
      == 0) // take symbol height as a reference in case there are no elements
    {
      double x = dx;
      double y = dy;
  ...
  // we have elements
  else
    {
      ...
      // provisional enum to indicate if we have to trace the left side or the
      // right side
      enum class Where
      {
        LEFT,
      ...
      if (where == Where::RIGHT)
        x += (reference->geometry.get_margin_X () * 2
              + reference->geometry.width)
             * reference->geometry.scale;
      else
        x -= reference->geometry.get_margin_X () * reference->geometry.scale;

      double y
          = dy + reference->geometry.y
            + reference->geometry.get_margin_Y () * reference->geometry.scale;
      ...
      cr->line_to (
          x,
          height
              - (y + reference->geometry.height * reference->geometry.scale));
      cr->stroke ();
    }
      ...
      if (el->type == ElementType::FRACTION)
        {
          ...
          double numerator_bottom
              = numerator->get_y ()
                - numerator->get_margin () * numerator->get_scale ();
          double denominator_top
              = denominator->get_y ()
                + (denominator->get_height () + denominator->get_margin ())
                      * denominator->get_scale ();

          // note: don't apply margins. margins are always applied by the
          // expression when calculating relative positions.

          double y
              = (numerator_bottom + denominator_top) / 2 + fe->geometry.y + dy;

          cr->move_to (dx + fe->geometry.x, height - y);
          cr->line_to (dx + fe->geometry.x
                           + fe->geometry.width * fe->geometry.scale,
                       height - y);
          cr->stroke ();

          draw_elements (cr, denominator,
                         dx + fe->geometry.x + denominator->get_x (),
                         dy + fe->geometry.y + denominator->get_y (), width,
                         height, cursor);
          draw_elements (cr, numerator,
                         dx + fe->geometry.x + numerator->get_x (),
                         dy + fe->geometry.y + numerator->get_y (), width,
                         height, cursor);
        }
      else if (el->type == ElementType::POWER)
        {
          ...
          draw_elements (cr, base, dx + pe->geometry.x + pe->base->get_x (),
                         dy + pe->geometry.y + pe->base->get_y (), width,
                         height, cursor);

          draw_elements (cr, exponent,
                         dx + pe->geometry.x + pe->exponent->get_x (),
                         dy + pe->geometry.y + pe->exponent->get_y (), width,
                         height, cursor);
        }
      else if (el->type == ElementType::SYMBOL)
        {
          ...
          double x = dx + se->geometry.x
                     + (g.width - extents.width) / (2 * se->geometry.scale);

          // if it is a dot, don't center it vertically.
          double y = dy + se->geometry.y
                     + ((se->symbol == ".") ? extents.height
                                            : (g.height - extents.height) / 2)
                           * se->geometry.scale;
          // show it
          cr->move_to (x, height - y);
...
void
Display::on_draw (const Cairo::RefPtr<Cairo::Context> &cr, int width,
                  int height)
{
